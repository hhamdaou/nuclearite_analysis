...
def get_gcd_and_files(runinfo):
	return " ".join( [ runinfo.get_gcd_file() ] + runinfo.get_files() )
...
for yr in detyrs:
	rn_count = 0
	for rn in runnums_phys[yr]:
		mmp.vbprint( "phys season {}, run counter {:4}".format(yr,rn_count), [1,2], 2 ) 
		rn_count += 1
		print_phys[yr] += get_gcd_and_files(grl[rn])
		time_phys[yr]  += grl[rn]["livetime"]
