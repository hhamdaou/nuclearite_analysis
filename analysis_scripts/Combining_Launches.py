...
#METAPROJECT /home/sdharani/i3_software/combo_stable/build/
...
if __name__ == "__main__":
    ...
    if int(args.Trigger)==1: #Triggering is optional
        ...
        # tray.Add("I3Writer", Filename=indir + args.outfile+"_Processed.i3.zst")
        #print(indir+args.outfile + '_Pocessed.hd5')
        print("/data/user/slatseva/BKG_simulation/Simulated_data_05_2016/"+args.outfile + '_Pocessed.hd5')

        #hdf = I3HDFTableService(os.path.join(indir, args.outfile+"_Processed.hd5"))
        hdf = I3HDFTableService(os.path.join("/data/user/slatseva/BKG_simulation/Simulated_data_05_2016/", args.outfile+"_Processed.hd5"))
        tray.AddModule(I3TableWriter,'writer', tableservice = hdf,   keys  = SlopVariableNames,  SubEventStreams = ["SLOPSplit"]  )
        '''

        #tray.Add("I3Writer", Filename=os.path.join("/data/user/slatseva/BKG_simulation/new/", "Run_"+args.year+"_"+args.month+"_"+args.outfile+".i3.zst"),
        tray.Add("I3Writer", Filename=os.path.join("/data/ana/BSM/IC86_SubRelativisticMonopoles/Background/new/Level_1/", "Run_"+args.year+"_"+args.month+"_"+args.outfile+".i3.zst"),
                Streams = [icetray.I3Frame.DAQ, icetray.I3Frame.Physics, 
                           icetray.I3Frame.DetectorStatus, icetray.I3Frame.Geometry]
                        )
        
    else:
        # tray.Add("I3Writer", Filename=indir + args.outfile+"_Recombined.i3.zst")
        #print(indir + args.outfile+"_Recombined.hd5")
        print("/data/user/slatseva/BKG_simulation/Simulated_data_05_2016/" + args.outfile+"_Recombined.hd5")

        #hdf = I3HDFTableService(os.path.join(indir, args.outfile+"_Recombined.hd5"))
        '''hdf = I3HDFTableService(os.path.join("/data/user/slatseva/BKG_simulation/Simulated_data_05_2016/", args.outfile+"_Recombined.hd5"))
        tray.AddModule(I3TableWriter,'writer', tableservice = hdf,   keys  = SlopVariableNames,  SubEventStreams = ["SLOPSplit"]  )
        '''
